...
import numpy as np
# 改模块运行会比较不同 M 对于拟合的影响,先后显示两个图像,分别是不同M值拟合的所有图像与不同M值对应得Erms(训练集和测试集)图像
# 通过修改循环的初始值和叠加值及终止值控制 M 的循环
start = 0
feed = 1
end = 9
...
plt.show()
# 下面是画 M 取值与Erms的关系
plt.title("Erms for different M")
plt.plot(M,ErmsTest,color="red",label="Test")
plt.plot(M,ErmsTraining,color="blue",label="Training")
plt.legend()
plt.show()
